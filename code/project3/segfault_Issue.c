...
int create_pipeline(char **left_args, char **right_args)
{
    ...
    if (pid2 == 0)
    {
        ...
        // Execute the second command
        exec_command(right_args, -1, NULL);
...
void execute_batch_file(FILE *fp, int *status)
{
    ...
    while (getline(&lineptr, &n, fp) != -1)
    {
        ...
        if (tokens != NULL)
        {
            ...
            {
                exec_command(tokens, num_tokens, status);
            }
        }
...
int execute_pipe(char **tokens, int argc, int *status)
{
    ...
    if (argc > 2 && strcmp(tokens[0], "cd") == 0)
    {
        ...
        if (strcmp(tokens[2], "|") == 0)
        {
            ...
            status = exec_command(new_tokens, argc - 3, status);
    ...
    if (IS_INTERACTIVE)
    {
        ...
        do
        {
            ...
            if (has_pipe)
            {
                ...
                execute_pipe(tokens, token_count, status);
            }
            else
            {
