...
/** A program that replicates a typical linux shell
 * @ author, Kalpesh Chavan & Hieu Nguyen
*/
...
/** String Tokenizer that checks for special characters
 * searches for redirections, pipes, new lines, nulls, spaces
 * 
 * @input, takes a char *line to check
 * @return char * of checked input values
*/
char **process_line(char *line)
{
...
/** executes a command where there are no pipe characters and returns if successful
 * creates a child to process
 * @input char **args, the arguments to pass and process
 * @input int argc, the number of arguemnts to pass and process
 * @return int, the status of if exec command succeeded or failed
 * 
*/
int exec_command(char **args, int argc)
{
...
/**takes found files and replaces wild card in command
 * @input **args, takes the argument inputs
 * @returns new_arguments, new list of arguments without wild card
 * 
*/
char **glob_eval(char **args)
{
 ...
 * Helper method to find_glob()
 * @input *filename, the filename to check
 * @input *pattern, the pattern to reference
 * @return int, returns comparison of file name to pattern if no asterisk, or if filename is of valid size and has the correct suffix 
*/
...
/**Function to find matches to a glob pattern (which contains a SINGLE wildcard), helper to glob_eval
 * Uses Matches_Pattern to look for asterisks and finds globs
 * @input *path, the path to be checked
 * @input *pattern, the pattern to be referenced
 * @return char **, the list of valid files
*/
char **find_glob(char *path, char *pattern)
{
...
/**Replicates the use of cd command
 * checks if there are additional args to change working dir, and does if needed
 * @input **args, the list of arguments to process with cd
 * @return if the command of CD is successful or not 
*/
int builtin_cd(char **args)
{
    if (args[1] == NULL)
...
/**Replicates the use of pwd command
 * checks the validity of getcwd and prints if valid
 * @input **args, the arguments to pass to process cwd
 * @return prints the working dir if success, exit if fails
 * 
*/
int builtin_pwd(char **args){
    char cwd[1024];
...
/**Replicates the use of which
 * executes commands with forks and waits for other commands
 * @input **args, the inputs to be processed
 * @return, processes the inputs and returns if successful or failed
 * 
*/
int builtin_which(char **args)
{
...
/**Replicates the use of exit command
 * only exits out of mysh
*/
void builtin_exit(char **args)
{
    exit(0);
...
/**Checks if input is a built in command (pwd, which, cd, exit) and executes
 * @input **line, the command to check
 * @return status, if the command was successful
*/
int is_builtin(char **line)
{
    int i;
 ...
 * reads line by line of the .sh file
 * processes the same way as IS_INTERACTIVE inside of main
 * checks for then, else, pipes and changes line processes as needed
 * @input *fp, the .sh file to be executed
 * 
*/
...
/**Executes arguments when there are pipes involved
 * Functions similarly to Execute_command()
 * handles forking and pipe differently
 * uses execute_command to execute left and right sides of pipes
 * @input **tokens, the inputs to be processed
 * @input argc, the number of tokens to process
 * @return, returns status if execute_pipe is successful or failed
 * 
*/
int execute_pipe(char **tokens, int argc)
{
...
/**Removes "then" or "else" from arguments list
 * removes "then" or "else" for further processing from the arguments list
 * @input **args, the arguements to be checked and modified
 * @input count, the count of arguments
 * 
*/
void fix_then_else(char **args, int count)
{
    ...
    // Free the memory pointed to by the first element if dynamically allocated
    free(args[0]);
...
/**Main method to run
 * Checks for batch or interactive modes, runs depending on input call
 * @return 0 to exit
*/
